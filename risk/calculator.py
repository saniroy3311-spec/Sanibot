...
def should_trigger_be(profit_dist: float, atr: float, be_mult: float = 1.0) -> bool:
    """
    Check if breakeven should trigger.
    Pine parity: profit_dist > atr * BE_MULT
    """
...
def upgrade_trail_stage(current_stage: int, peak_profit_dist: float, atr: float, entry_price: float = 0.0) -> int:
    """
    Upgrade trail stage based on profit distance.
    Uses TRAIL_STAGES config list for trigger multipliers.
    """
...
def compute_trail_sl(stage: int, peak_price: float, peak_profit_dist: float, is_long: bool, atr: float) -> Optional[float]:
    """
    Compute trailing SL for a given trail stage.
    Pine Script exact parity (matches monitor/trail_loop.py _compute_trail_sl).
    """
    from config import PINE_MINTICK, TRAIL_STAGES
...
def max_sl_threshold(atr: float) -> float:
    from config import MAX_SL_MULT, MAX_SL_POINTS
