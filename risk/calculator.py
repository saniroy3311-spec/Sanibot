...
def calc_trail_stage(profit_dist: float, atr: float) -> int:
    ...
    # --- DELTA SCALPER OFFER (0% EXIT FEE) ---
    @staticmethod
    def calculate_scalper_fee(entry_p: float, exit_p: float, btc_size: float, hold_duration_mins: float) -> tuple:
        entry_fee = (entry_p * btc_size) * 0.000236
        std_exit_fee = (exit_p * btc_size) * 0.000590
        if hold_duration_mins <= 30.0:
            exit_fee = 0.0
            is_scalper = True
        else:
            exit_fee = std_exit_fee
            is_scalper = False
        return entry_fee, exit_fee, entry_fee + exit_fee, is_scalper
